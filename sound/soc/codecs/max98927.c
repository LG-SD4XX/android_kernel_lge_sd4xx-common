...
static void max98927_check_esd_damage(struct snd_soc_codec *codec)
{
	/*
	 * We will check Rx Enables A and Tx Enables A registers whether these
	 * registers are initialized by default or not.
	 * If these are initialized by default(0x00), max98927 may get damage.
	 */
	struct max98927_priv *max98927 = snd_soc_codec_get_drvdata(codec);
	unsigned int rx_enables_a, tx_enables_a = 0x00;

	max98927_wrapper_read(max98927, MAX98927L,
			MAX98927_PCM_RX_ENABLES_A, &rx_enables_a);
	max98927_wrapper_read(max98927, MAX98927L,
			MAX98927_PCM_TX_ENABLES_A, &tx_enables_a);

	if (((rx_enables_a & 0xFF) != max98927_reg_map[4].def) ||
			((tx_enables_a & 0xFF) != max98927_reg_map[6].def))
	{
		msg_maxim("MAX98927 will be set to by dt values.");
		...
		struct snd_soc_dai *dai)
{
    ...
    max98927_wrapper_update(max98927, MAX98927B,
            MAX98927_PCM_MODE_CFG,
            MAX98927_PCM_MODE_CFG_PCM_CHANSZ_32, 0);
        ...
        struct snd_soc_dai *dai)
{
	...
	max98927_check_esd_damage(codec);
