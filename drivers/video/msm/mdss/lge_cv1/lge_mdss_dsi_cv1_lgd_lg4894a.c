...
struct mdss_panel_data *lgd_lg4894a_pdata_base;
EXPORT_SYMBOL(lgd_lg4894a_pdata_base);
...
int set_touch_osc(int enable)
{
	...
	if (set_touch_osc_flag) {
		if (!lgd_lg4894a_pdata_base) {
			pr_err("no panel connected!\n");
			return -EINVAL;
		}

		ctrl = container_of(lgd_lg4894a_pdata_base,
...
int lgd_lg4894a_mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	if (lge_mdss_dsi_panel_power_seq_all()) {
		...
		LGE_MDELAY(1);
		dw8768_register_set(0x03, 0x83); // fast discharging
...
int lgd_lg4894a_mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
		...
		if (lge_mdss_dsi_panel_power_seq_all()) {
			...
			dw8768_register_set(0x03, 0x80); // floating
			LGE_MDELAY(5);
...
void lgd_lg4894a_mdss_dsi_ctrl_shutdown(struct platform_device *pdev)
{
	...
	ret = mdss_dsi_panel_reset(lgd_lg4894a_pdata_base, 0);
	if (ret) {
		pr_warn("%s: Panel reset failed. rc=%d\n", __func__, ret);
		ret = 0;
	}
