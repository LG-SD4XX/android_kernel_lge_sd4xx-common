...
int status_vcom_mtp=1;
module_param_named(panel_status_vcom_mtp, status_vcom_mtp, int, 0);
MODULE_PARM_DESC(panel_status_vcom_mtp, "LCM with VCOM MTP or not");
	...
	if (enable) {
		if (!pinfo->cont_splash_enabled) {
			...
			if (lge_mdss_dsi_panel_power_seq_all()) {
				...
				ext_dsv_mode_change("output_voltage_mode_reg");
				ext_dsv_mode_change("positive_only_on_mode_reg");
			}
#endif
	...
	if (enable) {
		...
		if (!pinfo->cont_splash_enabled) {
			...
			if(lge_mdss_dsi_panel_power_seq_all()) {
				...
				ext_dsv_mode_change("normal_mode_reg");
...
#if IS_ENABLED(CONFIG_LGE_DISPLAY_EXTERNAL_DSV)
		ext_dsv_mode_change("power_off_mode_reg");
#endif
...
int mdss_dsi_panel_power_off_lgd_incell_sw49106(struct mdss_panel_data *pdata)
{
	...
	if (lge_mdss_dsi_panel_power_seq_all()) {
		ext_dsv_mode_change("power_off_mode_reg");
		ext_dsv_mode_change("power_off_hiz_mode_reg");
		ext_dsv_chip_enable(0);
	}
#endif
...
int lge_mdss_dsi_post_event_handler(struct mdss_panel_data *pdata, int event, void *arg)
{
	...
	case MDSS_EVENT_UNBLANK:
		if (panel_type == LGD_INCELL_SW49106) {
			if (lge_mdss_dsi_panel_power_seq_all()) {
#if IS_ENABLED(CONFIG_LGE_DISPLAY_EXTERNAL_DSV)
				ext_dsv_mode_change("normal_without_delay_mode_reg");
#endif
	...
	case MDSS_EVENT_BLANK:
		if (panel_type == LGD_INCELL_SW49106) {
			...
			if (!(lge_mdss_fb_get_shutdown_state() || lge_mdss_dsi_panel_power_seq_all()))
				ext_dsv_mode_change("lpwg_mode_reg");
#endif
