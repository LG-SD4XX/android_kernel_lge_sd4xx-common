...
#define LGE_PM_QNS_USE
#define READ_CURRENT_SIGN	(-1)
#define CHARGE_CURRENT_PROP		POWER_SUPPLY_PROP_QNS_CURRENT_MAX
#define CHARGE_VOLTAGE_PROP		POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE

#ifdef LGE_PM_QNS_USE
...
static int qns_set_ibat(int ibatmA)
{
		...
		if(ibat_psy->set_property(ibat_psy,	CHARGE_CURRENT_PROP, &propVal) != 0)
		{
...
{
#if 0 // Do not use "Float Voltage control".
		...
		if(ibat_psy->set_property(ibat_psy,	CHARGE_VOLTAGE_PROP, &propVal) !=0)
		{
...
static int qns_get_fcc(int *fcc, int *design)
{
		...
		if(bms_psy)
		{
				if(fcc != NULL)
				{
						if(bms_psy->get_property(bms_psy, POWER_SUPPLY_PROP_CHARGE_FULL, &ret) != 0)
						{
				...
				if(design != NULL)
				{
						if(bms_psy->get_property(bms_psy, POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN, &ret) != 0)
						{
...
static int qns_get_battery_type(const char **battery_type)
{
		...
		if (bms_psy)
		{
				...
				{
						if(bms_psy->get_property(bms_psy,
