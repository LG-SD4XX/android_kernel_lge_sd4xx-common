...
struct smbchg_chip {
	...
	struct delayed_work 	init_aicl_time;
#endif
#ifdef CONFIG_LGE_PM_CABLE_DETECTION
...
#endif
#ifdef CONFIG_MACH_MSM8940_L6_DCM_JP
	/* if ibat under 300 then charging off test*/
	THERMAL_BATCHG_EN_VOTER,
#endif
...
#ifdef CONFIG_LGE_PM
static int smbchg_debug_mask = PR_INTERRUPT|PR_STATUS|PR_MISC|PR_LGE|PR_TYPEC;
#else
...
{
	union power_supply_propval prop = {0, };
	int c_type = 0, c_mv, target_icl_ma;
	int rc;

	get_property_from_typec(chip,POWER_SUPPLY_PROP_CURRENT_MAX, &prop);
	if (chip->typec_current_ma == prop.intval)
		return;

	chip->typec_current_ma = prop.intval;
	target_icl_ma = get_effective_result_locked(chip->usb_icl_votable);
	if (chip->typec_current_ma == 0 ||
		target_icl_ma == chip->cfg_usb_max_current_ma ||
		chip->typec_current_ma == target_icl_ma) {
	...
	pr_smb(PR_LGE, "Type[%d], c_mV[%d], c_mA[%d], "
			"target_icl_mA[%d]\n", c_type, c_mv, chip->typec_current_ma, target_icl_ma);
	smbchg_parallel_usb_disable(chip);

	rc = vote(chip->usb_icl_votable, PSY_ICL_VOTER, true, chip->typec_current_ma);
	if (rc < 0)
...
#ifdef CONFIG_MACH_MSM8940_L6_DCM_JP
	/* if ibat under 300 then charging off test*/
	if(smbchg_ibat_hvdcp_limit < 300)
		ret |= vote(the_chip->battchg_suspend_votable, THERMAL_BATCHG_EN_VOTER, true, 0);
	else
		ret |= vote(the_chip->battchg_suspend_votable, THERMAL_BATCHG_EN_VOTER, false, 0);
#endif
...
static void charging_state_delay(struct work_struct *work)
{
	...
	/* if ibat under 300 then charging off test*/
	if(smbchg_ibat_hvdcp_limit < 300)
		rc |= vote(chip->battchg_suspend_votable, THERMAL_BATCHG_EN_VOTER, true, 0);
	else
		rc |= vote(chip->battchg_suspend_votable, THERMAL_BATCHG_EN_VOTER, false, 0);
#endif
...
#ifdef CONFIG_MACH_MSM8940_L6_DCM_JP
static void init_aicl(struct work_struct *work)
{
	int rc;

	struct smbchg_chip *chip =
				container_of(work, struct smbchg_chip, init_aicl_time.work);
	
	pr_smb(PR_LGE, " aicl_period = %d sec\n",chip->aicl_rerun_period_s);
...
#ifdef CONFIG_MACH_MSM8940_L6_DCM_JP 
	if (chip->aicl_rerun_period_s > 0) {
		rc = smbchg_set_aicl_rerun_period_s(chip, 23);
		if (rc < 0) {
			dev_err(chip->dev, "Couldn't set AICL rerun timer rc=%d\n",
					rc);
			return rc;
...
#endif
#ifdef CONFIG_MACH_MSM8940_L6_DCM_JP
	INIT_DELAYED_WORK(&chip->init_aicl_time, init_aicl);
#endif
#ifdef CONFIG_LGE_PM_CHARGING_TEMP_SCENARIO
...
#endif
#ifdef CONFIG_MACH_MSM8940_L6_DCM_JP
	schedule_delayed_work(&chip->init_aicl_time, 60*HZ);
#endif
#ifdef CONFIG_LGE_PM_CHARGING_TEMP_SCENARIO
