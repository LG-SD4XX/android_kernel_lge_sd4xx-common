	...
	uint32_t force_overflow)
{
	...
	if (overflow_mask) {
        ...
        /*LGE_CHANGE_S, Fixes for overflow recovery, 2017-06-20, dongsu.bag@lge.com */
        ...
        if (atomic_cmpxchg(&vfe_dev->error_info.overflow_state,
             ...
             pr_err("returning after cmpxchg for VFE%d!\n", vfe_dev->pdev->id);
		...
		if (vfe_dev->reset_pending == 1) {
			...
			*irq_status1 &= ~overflow_mask;
			spin_unlock_irqrestore(&vfe_dev->common_data->common_dev_data_lock, flags);/*LGE_CHANGE, Fixes for overflow recovery, 2017-06-20, dongsu.bag@lge.com */
			return;
		}
...
/*LGE_CHANGE_S, Fixes for overflow recovery, 2017-06-20, dongsu.bag@lge.com */
		...
		atomic_cmpxchg(&vfe_dev->error_info.overflow_state,
                       NO_OVERFLOW, OVERFLOW_DETECTED);
#endif
/*LGE_CHANGE_E, Fixes for overflow recovery, 2017-06-20, dongsu.bag@lge.com */
