...
int synaptics_vref_mode_check(struct device *dev)
{
	struct synaptics_data *d = to_synaptics_data(dev);
	int ret = 0;
	int vref_mode = 0;
	...
	ret = synaptics_read(dev, INCELL_EXT_DDI_CTRL_REG, buffer, sizeof(buffer));
	if (ret < 0) {
		TOUCH_E("failed to read vref status reg - ret:%d\n", ret);
		return ret;
	}

	vref_mode = buffer[21];
	TOUCH_I("%s - vref status %d\n", __func__, vref_mode);
...
static int synaptics_fw_compare(struct device *dev, const struct firmware *fw)
{
	...
	vref_check = synaptics_vref_mode_check(dev);

	if (ts->force_fwup) {
		update = 1;
	} else if ((device->major == 1 && device->minor == 9) && vref_check == 1) {
		update = 1;
