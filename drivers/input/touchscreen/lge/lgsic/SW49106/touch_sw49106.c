...
static int sw49106_tci_control(struct device *dev, int type)
{
	...
	case ACTIVE_AREA_CTRL:
                if (ts->lpwg.qcover != HALL_NEAR)
                    ...
                    ret = sw49106_tci_active_area(dev, 358, 684, 898, 1223); /*  quick cover Knock on/off area  */
		/*
				ts->tci.area.x1,
				ts->tci.area.y1,
				ts->tci.area.x2,
...
static void sw49106_debug_tci(struct device *dev)
{
	...
	TOUCH_I("[LPWG_DEBUG_FAIL_STATUS=[0x%x]\n", rdata);
		...
		if (bit_mask < TCI_FAIL_NUM) {
            ...
            TOUCH_I("Unkwon LPWG_DEBUG_FAIL_STATUS\n");
}
...
#if defined (CONFIG_MACH_MSM8940_L6_DCM_JP)
                        sw49106_setup_q_sensitivity(dev, 1);
                        // U3 active area setting
                        sw49106_qcover_active_area(dev, 358, 684, 898, 1223); /*  quick cover Knock on/off area  */
#endif
			}
		else {
			sw49106_tc_driving(dev, d->lcd_mode);
