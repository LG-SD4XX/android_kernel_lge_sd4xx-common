...
static int set_clks(struct fpc1028_data *fpc1028, bool enable)
{
	int rc = 0;
	mutex_lock(&fpc1028->lock);

	if (enable == fpc1028->clocks_enabled)
		goto out;

	if (enable) {
		rc = clk_prepare_enable(fpc1028->core_clk);
		if (rc) {
			dev_err(fpc1028->dev,
		...
		rc = clk_prepare_enable(fpc1028->iface_clk);
		if (rc) {
			dev_err(fpc1028->dev,
					"%s: Error enabling iface clk: %d\n",
					__func__, rc);
			clk_disable_unprepare(fpc1028->core_clk);
			goto out;
		}
		dev_dbg(fpc1028->dev, "%s ok. clk rate %u hz\n", __func__,
				fpc1028->spi->max_speed_hz);

		fpc1028->clocks_enabled = true;
	} else {
		clk_disable_unprepare(fpc1028->iface_clk);
		clk_disable_unprepare(fpc1028->core_clk);
		fpc1028->clocks_enabled = false;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	return set_clks(fpc1028, (*buf == '1')) ? : count;
}
...
*/
	...
	fpc1028->iface_clk = clk_get(dev, "iface_clk");
	if (IS_ERR(fpc1028->iface_clk)) {
		dev_err(dev, "%s: Failed to get iface_clk\n", __func__);
		rc = -EINVAL;
		goto exit;
	}

	fpc1028->core_clk = clk_get(dev, "core_clk");
	if (IS_ERR(fpc1028->core_clk)) {
...
//	rc = select_pin_ctl(fpc1028, "fpc1028_spi_active");
	...
	fpc1028->wakeup_enabled = false;
	fpc1028->clocks_enabled = false;
	fpc1028->clocks_suspended = false;
...
static int fpc1028_suspend(struct spi_device *spi, pm_message_t mesg)
{
	struct fpc1028_data *fpc1028 = dev_get_drvdata(&spi->dev);
	struct device *dev = &spi->dev;
//	fpc1028->clocks_suspended = fpc1028->clocks_enabled;
	if (of_property_read_bool(dev->of_node, "fpc,enable-wakeup")) {
...
//	if (fpc1028->clocks_suspended)
//		set_clks(fpc1028, true);
	return 0;
}
...
{
	int rc = spi_register_driver(&fpc1028_driver);
	if (!rc)
		pr_info("%s OK\n", __func__);
	else
		pr_err("%s %d\n", __func__, rc);
