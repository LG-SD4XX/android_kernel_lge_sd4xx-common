...
static int broadcast_dmb_open(void __user *arg)
{
    ...
    if(arg==NULL) {
        printk(KERN_ERR"[dtv] broadcast_dmb_open() arg is Null\n");
        return ERROR;
    }

    if(copy_from_user(&udata, arg, sizeof(struct broadcast_dmb_init_info)))
    {
        printk(KERN_ERR"[dtv] broadcast_dmb_open() copy_from_user fail!!! \n");
        rc = ERROR;
    }
...
static int broadcast_dmb_get_sig_info(void __user *arg)
{
    ...
    else
    {
        ...
        if(rc == ERROR) {
            printk(KERN_ERR"[dtv] broadcast_dmb_get_sig_info() fail!!! \n");
            return ERROR;
        }
