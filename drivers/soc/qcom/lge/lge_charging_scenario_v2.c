...
static int warm_ibat = 450;
static int cool_ibat = 450;
static int otp_ibat_limit;
...
void lge_set_otp_current(int otp_warm_ibat, int otp_cool_ibat)
{
	warm_ibat = otp_warm_ibat;
	cool_ibat = otp_cool_ibat;
	pr_err("LGE charging scenario V2 setting otp_limit to w:%d c:%d\n", warm_ibat, cool_ibat);
}
...
void lge_monitor_batt_temp(struct charging_info req, struct charging_rsp *res)
{
	...
	if(battemp_state > CHG_BATTEMP_12_43)
		otp_ibat_limit = warm_ibat;
	else
		otp_ibat_limit = cool_ibat;
	...
	if (charging_state == CHG_BATT_NORMAL_STATE) {
		...
		if(battemp_state <= CHG_BATTEMP_10_12) { // if low temperature
			if(req.batt_volt >= safe_bat_thr) {
				...
				res->dc_current = SAFETY_VOLTAGE_IBAT;
