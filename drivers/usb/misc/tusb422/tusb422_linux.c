...
#if defined(CONFIG_LGE_USB_TYPE_C) && defined(CONFIG_LGE_USB_MOISTURE_CORROSION_DETECT)
static ssize_t tusb422_moisture_detect_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	tcpc_device_t *tcpc_dev = tcpm_get_device(0);
	DEBUG("%s: moisture_detect_disable(%d)\n", __func__, tcpc_dev->moisture_detect_disable);
	return snprintf(buf, PAGE_SIZE, "%d\n", !tcpc_dev->moisture_detect_disable);
}


static ssize_t tusb422_moisture_detect_store(struct device *dev,
				       struct device_attribute *attr,
				       const char *buf, size_t count)
{
	tcpc_device_t *tcpc_dev = tcpm_get_device(0);
	...
	if (sscanf(buf, "%d", &moisture_detect) == 1) {
		tcpc_dev->moisture_detect_disable = !moisture_detect;
		DEBUG("%s: moisture_detect_disable(%d)\n", __func__, tcpc_dev->moisture_detect_disable);

		tcpc_dev->cc_swing_cnt = 0;
		tcpc_dev->cc_not_corroded = false;
		if (tcpc_dev->moisture_detect_disable) {
			PRINT("%s: Turn off moisture detection and DRP operation.\n", __func__);
			tcpc_dev->role = ROLE_SNK;
		} else {
			PRINT("%s: Turn on moisture detection and DRP operation.\n", __func__);
			tcpc_dev->role = ROLE_DRP;
		}
		return count;
	}
...
static DEVICE_ATTR(moisture_detect, S_IWUSR | S_IRUGO,
		   tusb422_moisture_detect_show, tusb422_moisture_detect_store);
#endif

static struct attribute *tusb422_attrs[] = {
	&dev_attr_registers.attr,
#if defined(CONFIG_LGE_USB_TYPE_C) && defined(CONFIG_LGE_USB_MOISTURE_CORROSION_DETECT)
	&dev_attr_moisture_detect.attr,
#endif
	NULL
};
